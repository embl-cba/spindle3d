...
public class Utils
{
	...
	{
		final ImgLabeling< Integer, IntType > imgLabeling = asImgLabeling( mask, ConnectedComponents.StructuringElement.EIGHT_CONNECTED );
			...
			int radius )
	{
		...
		final RandomAccessibleInterval< BitType > enlargedMask =
				getEnlargedRai( mask, radius );
		final RandomAccessibleInterval< BitType > enlargedMorphed =
				getEnlargedRai( morphed, radius );
	...
	RandomAccessibleInterval< R > dilate(
			...
			int radius )
	{
		final RandomAccessibleInterval< R > morphed = createEmptyCopy( image );
	...
	public static RandomAccessibleInterval< BitType > asMask( LabelRegion labelRegion )
	{
		...
		rai = getWithAdjustedOrigin( labelRegion, rai  );
	...
	double computeMaximum( final RandomAccessibleInterval< T > rai, double maxAxisDist )
	{
		...
		while ( cursor.hasNext() )
		{
			cursor.fwd();
			cursor.localize( position );
			if ( vectorLength( position ) <= maxAxisDist)
			{
								...
								ConnectedComponents.StructuringElement structuringElement )
	{
		final ArrayList< RegionAndSize > sortedRegions =
				getSizeSortedRegions(
						mask,
						structuringElement );
		setValues( mask, 0 );
		drawRegion( mask, sortedRegions.get( 0 ).getRegion(), 1.0 );
	}
	...
	RandomAccessibleInterval< T > createEmptyCellImg( RandomAccessibleInterval< T > volume )
	{
		...
		newImage = getWithAdjustedOrigin( volume, newImage );
		return newImage;
	}
	...
	public static double angleToXAxisInRadians( final double[] vector )
	{
		double[] xAxis = new double[]{ 1, 0, 0};

		double angleInRadians = getAngle( vector, xAxis );
